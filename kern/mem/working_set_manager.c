...
inline struct WorkingSetElement* env_page_ws_list_create_element(struct Env* e, uint32 virtual_address)
{
	...
	struct WorkingSetElement* new_ws = kmalloc(sizeof(struct WorkingSetElement));
	if(new_ws==NULL)
		panic("Cant't create element!");
	new_ws->virtual_address= ROUNDDOWN(virtual_address,PAGE_SIZE);
	uint32*ptr_page_table ;
	struct FrameInfo *ptr_frame  =get_frame_info(e->env_page_directory,virtual_address,&ptr_page_table) ;
	if(ptr_frame!=NULL)
		ptr_frame->environment = e, ptr_frame->element = new_ws;
	return new_ws;
}
