...
void free_page_ws(struct Env*curenv,struct WS_List*ws_list){
	struct WorkingSetElement * element,*next_iteration;
	if((&curenv->page_WS_list)==ws_list)
		element=LIST_FIRST(ws_list);
	else
		element=LIST_LAST(ws_list);
	while(element!=NULL && free_frame_list.size==0){
		if((&curenv->page_WS_list)==ws_list)
			next_iteration=LIST_NEXT(element);
		else
			next_iteration = LIST_PREV(element);
		if(pt_get_page_permissions(curenv->env_page_directory,element->virtual_address)&PERM_MODIFIED){
			if(curenv->disk_env_pgdir==NULL || curenv->disk_env_pgdir[PDX(element->virtual_address)]==0)
			{
				element=next_iteration;
				continue;
			}
		}
		LIST_REMOVE(ws_list,element);
		remove_victim(curenv,element->virtual_address);
		kfree(element);
		element=next_iteration;
	}
}
void remove_from_ready_queue(int choice){
	for(int i=0 ;i<num_of_ready_queues && free_frame_list.size==0; i++){
		struct Env* env;
		if(scheduler_method == SCH_BSD)
			env=env_ready_queues[i].lh_last;
		else
			env=env_ready_queues[i].lh_first;
		if(env!=NULL){
			if(choice==1)
				free_page_ws(env,&env->SecondList);
			else if(choice==2)
				free_page_ws(env,&env->ActiveList);
			else
				free_page_ws(env,&env->page_WS_list);
		}
	}
}
...
//		cprintf("\n Free Ram! \n");
		struct Env*tmp ;
		while((tmp=env_exit_queue.lh_last)!=NULL && free_frame_list.size==0){
			LIST_REMOVE(&env_exit_queue,tmp);
			env_free(tmp);
		}
		//2- otherwise, free at least 1 frame from the user working set by applying the FIFO algorithm
		remove_from_ready_queue(1);
		if(curenv!=NULL)
			free_page_ws(curenv,&curenv->SecondList);
		remove_from_ready_queue(2);
		remove_from_ready_queue(3);
		if(curenv!=NULL){
			free_page_ws(curenv,&curenv->ActiveList);
			free_page_ws(curenv,&curenv->page_WS_list);
		}
//		cprintf("\n Free second list has been done !\n");
		if(free_frame_list.size==0){
			if(curenv!=NULL){
				cprintf("\n allocate_frame cannot find a free frame.\n");
...
//			cprintf("\n Free RAM when it's FULL has been done!\n");
			*ptr_frame_info = LIST_FIRST(&free_frame_list);
		}
	}
...
//============================================================================================
//									un/Mapping
struct FrameInfo* Map_virtual(uint32 *ptr_dir,uint32 va,int perm){
	struct FrameInfo* ptr_frame;
	allocate_frame(&ptr_frame);
	va = ROUNDDOWN(va,PAGE_SIZE) ;
	if(map_frame(ptr_dir,(void*)ptr_frame,va,perm)==E_NO_MEM)
		free_frame(ptr_frame);
	else
		ptr_frame->va = va ;
	return ptr_frame ;
}
void unmap_range(uint32 st, uint32 en){
	for(uint32 va = st ;va<en;va+=PAGE_SIZE)
		unmap_frame(ptr_page_directory,va);
}
void map_range(uint32 st, uint32 en){
	for(uint32 va = st ;va<en;va+=PAGE_SIZE)
		if(Map_virtual(ptr_page_directory,va,PERM_AVAILABLE|PERM_WRITEABLE)==NULL)
			cprintf("\n error here !\n");
}
uint32 First_fit_in_table(uint32 *ptr_pag_dir,uint32 st_va , uint32 en_va , uint32 needed_pages){
	if(((en_va-st_va)>>12)<needed_pages)
		return 0 ;
	uint32 start =st_va,cnt=0;
	for(uint32 va = st_va; va!=en_va;va+=PAGE_SIZE){
		int perms = pt_get_page_permissions(ptr_pag_dir,va) ;
		if(perms==-1 || ((perms)&(Marked))==0)
			cnt++ ;
		else
			start=va+PAGE_SIZE , cnt=0 ;
		if(cnt==needed_pages)
			return start ;
	}
	return 0 ;
}
void Allocate_pages(uint32 *ptr_pag_dir,uint32 virtual_address, uint32 size,bool kernal){
	uint32 en = ROUNDUP(virtual_address+size,PAGE_SIZE);
	uint32 start_page = ROUNDDOWN(virtual_address,PAGE_SIZE) ;
	virtual_address=start_page;
	while(virtual_address!=en){
		uint32*ptr_page_table ;
		if(get_page_table(ptr_pag_dir,virtual_address,&ptr_page_table)==TABLE_NOT_EXIST)
			create_page_table(ptr_pag_dir,virtual_address);
		if(kernal)
			Map_virtual(ptr_pag_dir,virtual_address,PERM_WRITEABLE|Marked);
		else
			pt_set_page_permissions(ptr_pag_dir,virtual_address,Marked,0);
		virtual_address+=PAGE_SIZE ;
	}
	pt_set_page_permissions(ptr_pag_dir,start_page,START_OF_PAGE,0);
}
bool is_valid_page_address(uint32 *ptr_pag_dir,uint32 virtual_address){
	int perms = pt_get_page_permissions(ptr_pag_dir,virtual_address);
	if(perms==-1 || (perms&(IS_block_of_pages))!=(IS_block_of_pages))
		return 0 ;
	return 1 ;
}
void free_kernal_mem(uint32 *ptr_pag_dir,uint32 virtual_address){
	int perms;
	pt_set_page_permissions(ptr_pag_dir,virtual_address,0,START_OF_PAGE);
	do{
		//Unmark the given range to indicate it’s NOT reserved for the page allocator of this environment
		pt_set_page_permissions(ptr_pag_dir,virtual_address,0,Marked);
		unmap_frame(ptr_pag_dir,virtual_address);
		virtual_address+=PAGE_SIZE;
		perms = pt_get_page_permissions(ptr_pag_dir,virtual_address);
	}while(perms!=-1 && (perms&START_OF_PAGE)==0 && (perms&Marked)!=0);
}
