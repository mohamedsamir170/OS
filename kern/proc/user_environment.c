...
void remove_ws_element(struct Env * e,struct WS_List *ws_list,struct WorkingSetElement *element){
	unmap_frame(e->env_page_directory,element->virtual_address);
	LIST_REMOVE(ws_list,element);
	kfree(element);
}
void remove_ws_list(struct Env * e,struct WS_List *ws_list){
	while(!LIST_EMPTY(ws_list)){
		remove_ws_element(e,ws_list,ws_list->lh_last);
	}
}
...
//	cprintf("\n Free Env with ID: %d\n",e->env_id);
	...
	*/
		remove_ws_list(e,&e->ActiveList);
		remove_ws_list(e,&e->SecondList);
		remove_ws_list(e,&e->page_WS_list);
...
//			}
//			cprintf("PDX(USER_TOP): %d",PDX(USER_TOP));
			for(int i=0 ;i<PDX(USER_TOP);i++){
				uint32 page_directory_entry = e->env_page_directory[i] ;
				if(page_directory_entry&PERM_PRESENT)
					kfree((void *)kheap_virtual_address(EXTRACT_ADDRESS(page_directory_entry))) ;
...
void initialize_uheap_dynamic_allocator(struct Env* e, uint32 daStart, uint32 daLimit)
{
	...
	e->start = daStart , e->brk = daStart , e->Limit = daLimit ;
	e->nice_value = 0 ;
