...
void* sys_sbrk(int increment)
{
	...
	uint32 old_sbrk = curenv->brk ;
	void*res = (void*)update_sbreak(increment,&(curenv->brk),curenv->start,curenv->Limit);
	if(res!=(void*)-1){
		if(increment<0)
			free_user_mem(curenv,curenv->brk,(uint32)(-1*increment));
		else
			allocate_user_mem(curenv,old_sbrk,increment);
	}
	return res ;
}
/**************************************************************************/
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_free_user_mem:
		    if ((void*)a1 == NULL || (uint64)a1+a2 >= USER_LIMIT) {
	...
	case SYS_get_user_hard_limit:
		return curenv->Limit;
	case SYS_allocate_size_virtuallly:
		return First_fit_in_table(curenv->env_page_directory,curenv->Limit+PAGE_SIZE,USER_HEAP_MAX,convertSizeToNumberOfPages(a1));
	case SYS_free_size_virtually:
		if(a1>=curenv->Limit+PAGE_SIZE && a1<USER_HEAP_MAX && ((a1&(PAGE_SIZE-1))==0) && is_valid_page_address(curenv->env_page_directory,a1))
		{
			int perms;
			pt_set_page_permissions(curenv->env_page_directory,a1,0,START_OF_PAGE);
			a2 = a1 ;
			do{
				a1+=PAGE_SIZE, perms = pt_get_page_permissions(curenv->env_page_directory,a1);
