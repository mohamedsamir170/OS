...
void fault_handler(struct Trapframe *tf)
{
	...
	fault_va = rcr2();
//	if(curenv!=NULL && curenv->env_id==2051){
//		cprintf("\n Faulted VA = %x \n", fault_va);
//		print_trapframe(tf);
//	}
	...
	else
	{
		...
		if (userTrap)
		{
			...
			//your code is here
			if(fault_va>=USER_LIMIT || (fault_va>=USER_HEAP_START && fault_va<USER_HEAP_MAX &&
					((pt_get_page_permissions(faulted_env->env_page_directory,fault_va)&Marked)==0)) ||
					((perms & PERM_PRESENT)==1 && (perms&PERM_WRITEABLE)!=PERM_WRITEABLE))
		        sched_kill_env(faulted_env->env_id);
//			cprintf("\n Check if invalid addresse %x \n",fault_va);
//			if (fault_va>=USER_LIMIT){
//				cprintf("\n pointing to kernel -> Killed :( \n");
//		        sched_kill_env(faulted_env->env_id);
//			}
//			if(fault_va>=USER_HEAP_START && fault_va<USER_HEAP_MAX){
//				cprintf("\n check page in user heap 🤔 \n");
//				if((pt_get_page_permissions(faulted_env->env_page_directory,fault_va)&Marked)==0){
//					cprintf("\n pointing to unmarked page in user heap -> Killed :( \n");
//					sched_kill_env(curenv->env_id);
//				}
//			}
//			if((perms & PERM_PRESENT)==1 && (perms&PERM_WRITEABLE)!=PERM_WRITEABLE){
//				cprintf("\n exist but with read-only permissions :( -> Killed :( \n");
//		        sched_kill_env(faulted_env->env_id);
//			}
//			cprintf("\n passed :) ✅ \n");
			/*============================================================================================*/
