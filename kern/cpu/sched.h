...
#define PRI_MIN 0
#define PRI_MAX 63
#define ready_queue(p) for(int p = PRI_MAX ;p>=0 ;p--)
#define ready_env(p,ei) for(struct Env* ei = LIST_FIRST(&env_ready_queues[p]); ei!=NULL;ei= LIST_NEXT(ei))

int64 ticks;
uint32 ms ;
fixed_point_t _59_over_60,_1_over_60,c_recent;
int64 timer_ticks() ;
